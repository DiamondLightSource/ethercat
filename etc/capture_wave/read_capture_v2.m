...
% generated by monitor_capture
function capture = read_capture_v2(fname)
%
...
capture.fname = fname;
file = fopen(fname,'r');
checks.header = 'DATA DUMP FILE v2.0';
checks.tail = 'END DATA DUMP FILE v2.0';
...
capture.batchElements = fread(file, 1, 'int32');
capture.batchCount = fread(file, 1, 'int32');
capture.collectionElems = fread(file, 1, 'int32');
capture.missingElems = fread(file, 1, 'int32');
...
capture.data = fread(file, capture.batchElements, type);
tail = read_str(file);
