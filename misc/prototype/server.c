...
void ethercat_init()
{
    ...
    ec_pdo_entry_reg_t *entry = &domain1_regs[0];
    ...
    for(n = 0; n < MAX_CHANNELS; n++)
    {
        if (channel_map[n].channel == END_CHANNEL)
        {
            ...
            /* The registration array has to be terminated with an 
               empty structure, or one with the index field set to zero! */
            entry->index = 0;
            break;
        }
        ...
        {
            entry->alias = channel_map[n].alias;
            entry->position = channel_map[n].position;
            entry->vendor_id = channel_map[n].vendor_id;
            entry->product_code = channel_map[n].product_code;
            entry->index = channel_map[n].index;
            entry->subindex = channel_map[n].subindex;
            entry->offset = &channel_map[n].offset;
            entry->bit_position = &channel_map[n].bit_position;
            entry++;
        }
    }
