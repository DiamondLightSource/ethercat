...
FAIL this is premature optimization, see below. Just send the name.
...
Don't duplicate what can be done at runtime (yet)

1) run configtest to generate structure
Config file looks like this:

device name vendor product revision assign_activate
 pdo 0 name0 length
 entry 1 name1 length
end

device EL3702 0x00000002 0x0e763052 0x00020000 0x0730
 entry 0 cycle0 1
 entry 1 value0 100
 entry 2 cycle1 1
 entry 3 value1 100
...
Option to have UID as well as NAME (makes connection easier)
============================================================

Dynamically building the mapping, here we call the PDO entry "ID" and 1
Makes for a quicker prototype (names come later)

pdo_entry_type regs[] =
{
    { "ID", 1, 0x6000, 0x01, U16 },
    { NULL }
};

Then again name lookup won't be that slow, keep it stateless. Writes are less common.

We only need to look up the X parameters in our device. That's a couple of string comparisons.
The only slow part will be sending the string name in the write packet!
But it's stateless!

XML Config File
===============

Moving to XML, libxml2 is easy to use and allows upgrade to beckhoff config if
required.
